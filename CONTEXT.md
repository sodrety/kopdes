...
**Loan Type**:
The financing category selected by a **Member** before entering the requested amount for a **Loan Request**.
_Avoid_: A category assigned or changed by an Officer during approval

**Regular Loan**:
A **Loan Type** with a Member-selected tenor from one through twenty-four months and a tiered monthly **Admin Fee** multiplied by that tenor.
_Also called in UI_: Pinjaman Reguler

**Secondary Goods Loan**:
A **Loan Type** for financing secondary goods with a Member-selected tenor from one through twelve months and a one-time twenty-percent **Admin Fee**.
_Also called in UI_: Pinjaman Barang Sekunder

**Goods Purchase/Paylater Loan**:
A **Loan Type** for a goods purchase whose tenor is fixed at exactly one month and whose **Admin Fee** is five percent.
_Also called in UI_: Pembelian Barang/Paylater

**Legacy Loan Terms**:
The preserved financial terms of a historical **Regular Loan** whose obligation predates the current type-based **Admin Fee** policy.
_Also called in UI_: Ketentuan Lama
_Avoid_: Recalculating a historical obligation under the current Regular Loan fee tiers

**Requested Amount**:
The principal a **Member** wants to receive through a **Loan Request**, excluding the **Admin Fee**.
_Also called in UI_: Nominal Pinjaman
_Avoid_: Total loan, Total Obligation, amount including Admin Fee

**Proposed Loan Terms**:
The amount, tenor, automatically derived **Admin Fee** policy and amounts, **Total Obligation**, and start date fixed by **Manager** for the remaining **Approval Stages** of a **Loan Request**.
_Avoid_: Treating the Member's requested amount or tenor as an approval ceiling, manually entering an Admin Fee percentage, allowing a later Officer to alter terms that earlier Officers approved
...
**Admin Fee**:
A principal-based charge whose rate and one-time or monthly application are determined by the **Loan Type**, rounded to whole Rupiah using half-up rounding only when its calculation produces a fractional Rupiah.
_Also called in UI_: Biaya admin
_Avoid_: Bunga, interest, Officer override, assuming every Loan Type multiplies the charge by tenor, declining-balance charge, penalty
...
The expected monthly **Angsuran** rows generated by dividing an approved **Loan**'s **Total Obligation** as evenly as possible into whole Rupiah across its tenor, with any division remainder in the final installment.
...
**Adjustment Due**:
The internal status for a historically paid **Loan** reopened by a retroactive **Admin Fee**. It carries a Remaining Balance but does not count as the Member's one active Loan.
_Avoid_: Marking multiple historical Loans active

**Remaining Balance**:
The unpaid amount still owed on a **Loan**, including approved principal and scheduled **Admin Fee**.
_Avoid_: Outstanding account balance, principal-only balance

**Total Obligation**:
The approved principal plus the total **Admin Fee** owed across a **Loan**'s full tenor.
_Avoid_: Requested Amount, total loan
...
- A **Requested Amount** excludes the **Admin Fee** and represents the principal the Member wants to receive.
- A **Requested Amount** may be any positive whole-Rupiah amount regardless of **Loan Type**.
- A **Regular Loan** allows the Member to select a tenor from one through twenty-four months and applies its tiered **Admin Fee** monthly across that tenor.
- A **Regular Loan** combines its one-percent charge on the first Rp25,000,000 and its one-and-a-half-percent charge on approved principal above Rp25,000,000 before rounding the monthly **Admin Fee** once.
- A **Secondary Goods Loan** allows the Member to select a tenor from one through twelve months and applies a one-time **Admin Fee** equal to twenty percent of approved principal.
- A **Secondary Goods Loan** spreads its approved principal and one-time **Admin Fee** evenly across its **Installment Schedule**.
- A **Goods Purchase/Paylater Loan** always has a one-month tenor and applies an **Admin Fee** equal to five percent of approved principal.
- Every **Loan Type** represents a monetary amount disbursed to the Member; goods-related Loan Types do not represent a product, vendor payment, or inventory record.
- A historical **Regular Loan** with **Legacy Loan Terms** displays its actual preserved **Admin Fee** rather than the current Regular Loan fee rule.
- A submitted **Loan Request** retains its selected **Loan Type** throughout the **Approval Chain**.
...
- **Manager** may set the proposed amount and tenor either above or below the Member's requested amount and tenor.
- A proposed tenor must remain within the limit of the selected **Loan Type**.
- The proposed **Admin Fee** is derived from the selected **Loan Type**, proposed amount, and proposed tenor and is not entered as a percentage by **Manager**.
- No **Officer** may override a **Loan Type**'s **Admin Fee** rate, tier threshold, or one-time or monthly application.
- Manager approval snapshots the selected **Loan Type**, applied **Admin Fee** policy and amounts, and **Total Obligation** so later policy changes do not alter the remaining **Approval Stages** or resulting **Loan**.
- Manager-modified **Proposed Loan Terms** do not require a separate acceptance decision from the Member before the **Approval Chain** continues.
- A Member can view Manager-modified **Proposed Loan Terms**, including the proposed amount, tenor, **Admin Fee**, and **Total Obligation**.
- When **Manager** changes the requested amount or tenor, the Member receives an immediate notification containing the modified **Proposed Loan Terms** and a path to cancel the request.
- A Member may cancel a pending **Loan Request** after **Manager** sets different **Proposed Loan Terms** and before **Final Approval**.
...
- A cancelled **Loan** is a void obligation: it has no **Admin Fee**, schedule, or Remaining Balance and never blocks a new **Loan Request**.
- A **Loan** has one generated **Installment Schedule**.
- A **Loan** has many **Repayment Records**.
- A **Loan** may include an **Admin Fee** as part of its scheduled **Angsuran**.
- A **Repayment Record** may be partial, exact, or extra relative to scheduled **Angsuran** rows.
- **Repayment Records** cover the oldest unpaid **Installment Schedule** row first; partial coverage remains unpaid and may become overdue without adding penalties or additional **Admin Fees**.
...
- "Bunga" previously described the charge on a **Loan**. Resolved: use **Admin Fee** because the cooperative treats it as an administrative charge whose rate and one-time or monthly application depend on the **Loan Type**, rather than as interest.
...
- "Bunga" can mean flat interest, declining-balance interest, penalty, or fee. Resolved: use **Admin Fee** for the principal-based charge determined by **Loan Type**.
- "Remaining Balance" can mean principal-only or total owed. Resolved: **Remaining Balance** includes approved principal and scheduled **Admin Fee**.
