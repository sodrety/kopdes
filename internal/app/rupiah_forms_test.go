...
func TestGroupedRupiahBrowserFormsPersistExactAmounts(t *testing.T) {
	...
	loanRequestResponse := postForm("/api/member/loan-requests", memberToken, url.Values{
		"loan_type": {"regular"}, "requested_amount": {"Rp 5.000.000"},
	...
	if err := fixture.db.QueryRow(`SELECT id,requested_amount FROM loan_requests WHERE member_id=$1 AND status='pending'`, member.ID).Scan(&loanRequestID, &requestedAmount); err != nil || requestedAmount != 5_000_000 {
		t.Fatalf("Loan requested amount = %d, %v; want 5000000", requestedAmount, err)
	}
	loan := fixture.approveLoanRequest(t, adminToken, loanRequestID, 5_000_000, 5)
