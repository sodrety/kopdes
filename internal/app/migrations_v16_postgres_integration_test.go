...
func insertPostgresPendingSnapshottedLoanRequest(t *testing.T, db *sql.DB, requestID string, amount int64, duration int, monthlyFee, totalFee, obligation int64, nextStage string) {
	t.Helper()
	insertPostgresOriginLoanRequest(t, db, requestID, amount, duration, "Correct policy")
	insertPostgresLoanRequestApproval(t, db, requestID, "manager")
	if _, err := db.Exec(`UPDATE loan_requests SET current_approval_stage=$7,proposed_approved_amount=$2,proposed_duration_months=$3,proposed_start_date='2026-01-01',proposed_admin_fee_policy='regular_tiered_monthly_v1',proposed_monthly_admin_fee=$4,proposed_total_admin_fee=$5,proposed_total_obligation=$6 WHERE id=$1`, requestID, amount, duration, monthlyFee, totalFee, obligation, nextStage); err != nil {
...
func insertPostgresApprovedLoanRequest(t *testing.T, db *sql.DB, requestID string, amount int64, duration int, monthlyFee, totalFee, obligation int64) {
	t.Helper()
	insertPostgresPendingSnapshottedLoanRequest(t, db, requestID, amount, duration, monthlyFee, totalFee, obligation, "ketua_i")
