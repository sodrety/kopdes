...
func (s *Server) insertMember(req memberRequest) (Member, error) {
	...
	if member.MemberNo == "" || member.FullName == "" || member.JoinDate == "" || !validMemberStatus(member.Status) {
		return Member{}, errInvalidMember
	}

	_, err := s.db.Exec(
		`INSERT INTO members (id, member_no, full_name, phone, address, join_date, status) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		member.ID,
		member.MemberNo,
		member.FullName,
		member.Phone,
...
func (s *Server) allMembers() ([]Member, error) {
	rows, err := s.db.Query(`SELECT id, member_no, full_name, phone, address, join_date, status FROM members ORDER BY member_no`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var member Member
		if err := rows.Scan(&member.ID, &member.MemberNo, &member.FullName, &member.Phone, &member.Address, &member.JoinDate, &member.Status); err != nil {
			return nil, err
		}
