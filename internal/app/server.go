...
func NewServer(cfg Config, db *sql.DB) http.Handler {
	...
	admin.POST("/loan-requests/on-behalf", server.requireRole("admin"), server.createAdminLoanRequest)
	admin.GET("/loan-requests/template.xlsx", server.requireRole("admin"), server.downloadAdminLoanRequestTemplate)
	admin.POST("/loan-requests/bulk/preview", server.requireRole("admin"), server.adminLoanRequestBatchPreview)
	admin.POST("/loan-requests/bulk/:id/commit", server.requireRole("admin"), server.adminLoanRequestBatchCommit)
	admin.GET("/loan-request-batches/:id", server.requirePermission(PermissionRequestsView), server.adminLoanRequestBatchAPI)
	admin.GET("/loan-request-batches/:id/export.xlsx", server.requirePermission(PermissionRequestsView), server.exportAdminLoanRequestBatch)
	admin.POST("/loan-requests/:id/cancel", server.requirePermission(PermissionRequestsView), server.cancelAdminLoanRequest)
	...
	admin.POST("/transactions/source", server.requirePermission(PermissionTransactionSourceEdit), server.updateCashTransactionSources)
	admin.GET("/coa/accounts", server.requirePermission(PermissionCOAManage), server.adminCOAAccounts)
	admin.POST("/coa/import", server.requirePermission(PermissionCOAManage), server.importCOADraft)
	admin.GET("/coa/import/:id", server.requirePermission(PermissionCOAManage), server.coaImportBatch)
	admin.POST("/coa/import/:id/activate", server.requirePermission(PermissionCOAManage), server.activateCOADraft)
	admin.GET("/accounting-mappings", server.requirePermission(PermissionCOAManage), server.accountingMappings)
	admin.POST("/accounting-mappings", server.requirePermission(PermissionCOAManage), server.saveAccountingMapping)
	admin.POST("/accounting-mappings/:id/update", server.requirePermission(PermissionCOAManage), server.saveAccountingMapping)
	...
	admin.GET("/exports/transactions.csv", server.requirePermission(PermissionTransactionsView), server.exportTransactionsCSV)
	...
	member.POST("/profile/bank-details", server.updateOwnMemberBankDetails)
	...
	router.GET("/admin/loan-request-batches/:id", server.requirePermission(PermissionRequestsView), server.adminLoanRequestBatchPage)
	...
	router.GET("/admin/coa", server.requirePermission(PermissionCOAManage), server.adminCOAPage)
	router.GET("/admin/transactions", server.requirePermission(PermissionTransactionsView), server.adminTransactionsPage)
	router.GET("/admin/transactions/categories", server.requirePermission(PermissionTransactionCategoriesManage), server.adminTransactionCategoriesPage)
	...
	router.GET("/member/exports/savings.pdf", server.requireRole("member"), server.memberSavingsSlipPDF)
	router.GET("/member/exports/loans.pdf", server.requireRole("member"), server.memberLoanSlipPDF)
