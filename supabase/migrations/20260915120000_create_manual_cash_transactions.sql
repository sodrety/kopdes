-- PostgreSQL mirror of runtime migration 23 in internal/app/migrations.go.
BEGIN;
SET LOCAL statement_timeout = '30s';

ALTER TABLE public.officer_appointments
...
CREATE TABLE public.cash_transaction_categories (
    id TEXT PRIMARY KEY,
...
CREATE UNIQUE INDEX idx_cash_transaction_categories_direction_name
    ON public.cash_transaction_categories(direction, lower(name));

CREATE TABLE public.manual_cash_transaction_sequences (
    transaction_date TEXT PRIMARY KEY,
    next_sequence INTEGER NOT NULL CHECK (next_sequence > 0)
);

CREATE TABLE public.manual_cash_transactions (
    id TEXT PRIMARY KEY,
    transaction_date TEXT NOT NULL,
    direction TEXT NOT NULL CHECK (direction IN ('cash_in', 'cash_out')),
    category_id TEXT NOT NULL REFERENCES public.cash_transaction_categories(id),
...
CREATE INDEX idx_manual_cash_transactions_date
    ON public.manual_cash_transactions(transaction_date, created_at);
CREATE INDEX idx_manual_cash_transactions_direction_category
    ON public.manual_cash_transactions(direction, category_id);

CREATE TABLE public.cash_transaction_category_audits (
    id TEXT PRIMARY KEY,
    category_id TEXT NOT NULL REFERENCES public.cash_transaction_categories(id),
...
CREATE INDEX idx_cash_transaction_category_audits_category
    ON public.cash_transaction_category_audits(category_id, created_at);

INSERT INTO public.cash_transaction_categories (id, direction, name)
VALUES
...
CREATE FUNCTION public.protect_manual_cash_transactions_immutable() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
    RAISE EXCEPTION 'manual cash transactions are immutable';
END $$;
CREATE TRIGGER protect_manual_cash_transactions_immutable
BEFORE UPDATE OR DELETE ON public.manual_cash_transactions
FOR EACH ROW EXECUTE FUNCTION public.protect_manual_cash_transactions_immutable();

CREATE FUNCTION public.protect_cash_transaction_category_name() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
    IF EXISTS (SELECT 1 FROM public.manual_cash_transactions WHERE category_id = OLD.id)
...
BEFORE UPDATE OF name ON public.cash_transaction_categories
FOR EACH ROW EXECUTE FUNCTION public.protect_cash_transaction_category_name();

ALTER TABLE public.cash_transaction_categories ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.manual_cash_transaction_sequences ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.manual_cash_transactions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.cash_transaction_category_audits ENABLE ROW LEVEL SECURITY;
